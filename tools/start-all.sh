...
$CONTAINER_RUNTIME run --rm -d -e POSTGRES_SERVICE_HOST=$HOST -e POSTGRES_DB=$PGSQL_DB -e POSTGRES_USER=$PGSQL_USER -e POSTGRES_PASSWORD=$PGSQL_PASSWORD -p 8080:8080 --name backend $BACKEND_IMAGE

#$CONTAINER_RUNTIME run --rm -d -e BACKEND_PROVIDER_URL=remote+http://$(hostname):8080 -p 8180:8080 --name frontend $FRONTEND_IMAGE
$CONTAINER_RUNTIME run --rm -d -e BACKEND_PROVIDER_URL=remote+http://$HOST:8080 -p 8180:8080 --name frontend $FRONTEND_IMAGE

...
#$CONTAINER_RUNTIME run --rm -d --name pgdb -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=mypassword123 -e POSTGRES_DB=postgres -p 5432:5432 $POSTRES_IMAGE
...
$CONTAINER_RUNTIME run --rm -d -e POSTGRES_SERVICE_HOST=$(hostname) -e POSTGRES_DB=$PGSQL_DB -e POSTGRES_USER=$PGSQL_USER -e POSTGRES_PASSWORD=$PGSQL_PASSWORD -p 8080:8080 --name backend quay.io/bfarr/jboss-demo-backend
